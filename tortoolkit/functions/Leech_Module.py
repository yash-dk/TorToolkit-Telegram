...
async def check_link(msg,rclone=False,is_zip=False, extract=False):
    ...
    if msg is None:
        ...
        if msg.raw_text.lower().startswith("magnet:"):
            ...
            #Zippyshare revert back to old version for ttk by @yourtulloh
            ...
            # Here you can add blocklist link.
            elif 'drive.google.com' in urls or 'uptobox.com' in urls or 'mega.nz' in urls or '1fichier.com' in urls:
                await omess.reply("Unsupported link!")
                torlog.info("Unsupported link: {msg.raw_text}")
                return
            # Here you can block your index link. 
            elif 'yourIndexLink.workers.dev' in urls:
                await omess.reply("My index link can't be leech! Jembodmou.")
                torlog.info("Index link under attack: {msg.raw_text}")
                return
